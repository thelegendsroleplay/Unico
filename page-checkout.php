...
jQuery(document).ready(function($) {
    ...
    // File Upload handling for receipt
    $('.unico-upload-input').on('change', function() {
        var file = this.files[0];
        if (file) {
            var formData = new FormData();
            formData.append('file', file);
            formData.append('action', 'unico_upload_payment_receipt');
            // You might need a nonce for upload too
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        $('.unico-upload-placeholder').html('<span>' + file.name + ' uploaded</span>');
                    } else {
